		...
		i32 alpha, i32 beta, bool pv_node, u64 key) {
			...
			for (i32 i = 0; i < count && !stopped.load(std::memory_order_relaxed); ++i) {
				...
				if (depth >= 2 && move_index >= 1 + 2 * root) {
					...
					score = -negamax(next, info, searched_depth, ply + 1, -alpha - 1, -alpha, false, child_key);
					
					if (!stopped.load(std::memory_order_relaxed) && score > alpha && searched_depth < new_depth) {
						score = -negamax(next, info, new_depth, ply + 1, -alpha - 1, -alpha, false, child_key);
					}
				}
				...
				if (pv_node && (move_index == 0 || score > alpha)) {
					score = -negamax(next, info, new_depth, ply + 1, -beta, -alpha, true, child_key);
				}
