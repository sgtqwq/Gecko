...
namespace UCI {
	...
	void loop() {
		std::cout << "Gecko 0.14 by Bingwen Yang(sgtqwq)" << std::endl;
		// UCI::pos is a global object, so its constructor can run before main()
		// (and before Eval::init). Refresh incremental eval state here.
		pos.refresh_eval();
